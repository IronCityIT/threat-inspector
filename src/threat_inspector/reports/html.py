...
def generate_html_report(
    ...
    # Everything below is interpolated into HTML. `client_name` and
    # `project_name` arrive from the API request body, and a finding's severity,
    # port and compliance mappings come from the uploaded scan file — all of it
    # third-party text. Escape once, here, so no interpolation site below has to
    # remember to.
    client_name_html = _escape_html(client_name)
    project_name_html = _escape_html(project_name) if project_name else ""
    company_name_html = _escape_html(company_name)
    counts = {
        key: _as_count(summary.get(key))
        for key in (
...
<head>
    ...
    <title>Vulnerability Assessment Report - {client_name_html}</title>
    <style>
        :root {{
            --critical: #dc2626;
            --high: #ea580c;
...
<body>
    <header>
        <div class="container">
            ...
            <div class="subtitle">{client_name_html}{f" - {project_name_html}" if project_name else ""}</div>
            <div class="meta">
                <div>Generated: {datetime.now().strftime("%B %d, %Y at %H:%M")}</div>
                <div>Prepared by: {company_name_html}</div>
            </div>
        </div>
    </header>
    ...
    <div class="container">
        <div class="summary-grid">
            <div class="summary-card">
                <div class="count">{counts["total_vulnerabilities"]}</div>
                <div class="label">Total Findings</div>
            </div>
            <div class="summary-card critical">
                <div class="count">{counts["critical_count"]}</div>
                <div class="label">Critical</div>
            </div>
            <div class="summary-card high">
                <div class="count">{counts["high_count"]}</div>
                <div class="label">High</div>
            </div>
            <div class="summary-card medium">
                <div class="count">{counts["medium_count"]}</div>
                <div class="label">Medium</div>
            </div>
            <div class="summary-card low">
                <div class="count">{counts["low_count"]}</div>
                <div class="label">Low</div>
            </div>
            <div class="summary-card info">
                <div class="count">{counts["info_count"]}</div>
        ...
        <section>
            ...
            <p>
                ...
                conducted for {client_name_html}. A total of <strong>{counts["total_vulnerabilities"]}</strong>
                vulnerabilities were identified across the scanned assets.
            </p>
            <p style="margin-top: 1rem;">
                {"<strong style='color: var(--critical)'>Immediate attention is required</strong> for " + str(counts["critical_count"]) + " critical vulnerabilities that pose significant risk to the organization." if counts["critical_count"] > 0 else "No critical vulnerabilities were identified during this assessment."}
            </p>
        </section>
...
"""
    ...
    for vuln in sorted_vulns[:50]:  # Limit to 50 in summary table
        ...
        html_content += f"""
                    <tr>
                        <td><span class="severity-badge severity-{_severity_class(vuln.severity)}">{_escape_html(vuln.severity)}</span></td>
...
"""
    ...
    for i, vuln in enumerate(sorted_vulns, 1):
        ...
        if include_compliance and vuln.raw_data.get("compliance_mappings"):
            tags = [
                f"{m['framework']}-{m['requirement']}" for m in vuln.raw_data["compliance_mappings"]
            ]
            compliance_html = f"""
                <div class="compliance-tags">
                    {"".join(f'<span class="compliance-tag">{_escape_html(tag)}</span>' for tag in tags[:5])}
                </div>
"""
...
"""
        ...
        html_content += f"""
            <div class="vuln-detail {_severity_class(vuln.severity)}">
                <h3>
                    <span class="severity-badge severity-{_severity_class(vuln.severity)}">{_escape_html(vuln.severity)}</span>
                    {_escape_html(vuln.title)}
                </h3>
                <div class="meta">
                    <span><strong>Asset:</strong> {_escape_html(vuln.asset_ip or vuln.asset_name or "N/A")}</span>
                    {f"<span><strong>Port:</strong> {_escape_html(vuln.asset_port)}</span>" if vuln.asset_port else ""}
...
"""
        ...
        <section>
            ...
            <ol style="margin-top: 1rem; padding-left: 1.5rem;">
                <li style="margin-bottom: 0.5rem;"><strong>Address Critical Vulnerabilities Immediately:</strong>
                    Focus on the {counts["critical_count"]} critical findings that pose the highest risk.</li>
    ...
    <footer>
        <p>This report was generated by {company_name_html} using Iron City Threat Inspector.</p>
...
def _escape_html(text: object) -> str:
    ...
    Deliberately takes `object`: callers pass a finding's port (an int) and
    fields that may be None, and every one of them must be escaped. Narrowing
    this to `str` is what let the non-string interpolation sites skip escaping.
    """
    if not text:
        return ""
    return (
...
# The five severities the stylesheet defines a colour for. A finding's severity
# is whatever the uploaded scan file said it was, and it is used as a CSS class,
# so it is resolved against this set rather than interpolated.
_SEVERITY_CLASSES = frozenset({"critical", "high", "medium", "low", "info"})


def _severity_class(severity: str) -> str:
    """Map a finding's severity onto a known CSS class, or 'info' if unknown."""
    value = str(severity or "").strip().lower()
    return value if value in _SEVERITY_CLASSES else "info"


def _as_count(value: object) -> int:
    """Coerce a caller-supplied summary count to an int; 0 if it is not one."""
